...
/*   Updated: 2025/12/12 21:18:58 by vtaniga          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		char *env_value)
{
	...
	new_str_len = prefix_len + env_value_len + suffix_len + 1;
	new_str = (char *)malloc(new_str_len);
	if (!new_str)
		return (NULL);
	ft_strlcpy(new_str, original, prefix_len + 1);
	ft_strlcpy(new_str + prefix_len, env_value, env_value_len + 1);
	ft_strlcpy(new_str + prefix_len + env_value_len, dollar_pos + 1 + var_len,
		suffix_len + 1);
	return (new_str);
}
